...
async function main() {
    ...
    // --------------- create users ----------------------
        ...
        let megancromp = await createKCUser({
            ...
            description: "I'm a wildlife biologist currently studying sea turtles in the Caribbean. I'm using my skills in web development to help conservation orgs update their sites #ConservationOptimist"
        },{
            include: [{
    ...
    // --------------- create Campaigns ----------------------
        ...
        let carribeanCampaignRef2 = await db.campaigns.create({
            ...
            {
                description: `Thank you to [@leonargriffiths](/users/${leonardgriffiths.id}) for donating the gas money and to @virginatlantic for the In-person help we needed to save this nestlings! #CST #carribean`,
                displayLocation: ["Batts Rock Beach"],
                ...
                description: `Thank you to [@leonargriffiths](/users/${leonardgriffiths.id}) for donating the gas money and to [@natalieWeekes](/users/${natalieweekes.id}) for the In-person help we needed to save this nestlings! #CST #carribean`,
                displayLocation: ["Batts Rock Beach"],
            ...
            skillNeeds: [{
                name: 'Boating',
                description: "We need boat transportation ASAP for one Hawksbill sea turtle and two CST biologists from Bridgetown Harbor, carribean yo Egmont Harbor on Grenada. If you can only do one way is ok too.",
        ...
        let batCampaignRef = await db.campaigns.create({
            ...
            updatedAt:date,
            campaignPosts: [{
                description: `Endangered Wildlife Trust Thank you to [@leonardgriffiths](/users/${leonardgriffiths.id}) for donating the gas money and to [@natalieweekes](/users/${natalieweekes.id}) for the In-person help we needed to save this nestlings! #CST #carribean`,
                displayLocation: ["Surrey, United Kingdom"],
                mediaPath: "file_1542234784642_Screenshot from 2018-11-14 17-32-42.png",
                createdAt:date, 
                updatedAt:date,
    ...
    // --------------- tmp vars ----------------------
        let carribeanCampaign = await db.campaigns.findOne({
            where: {id: carribeanCampaignRef2.id},
            include: campaignIncludes
        });
    ...
    // --------------- create Support ----------------------
        await createSupport(caribbeanseaturtleproject, megancromp, carribeanCampaign, { type: 'skill', id: carribeanCampaign.skillNeeds[0].id, name: carribeanCampaign.skillNeeds[0].name });
        ...
        await createSupport(caribbeanseaturtleproject, leonardgriffiths , carribeanCampaign , { type:'inPerson' , id:carribeanCampaign.inPersonNeeds[0].id }) ;
        await createSupport(caribbeanseaturtleproject, megancromp , carribeanCampaign , { type:'money' , id:carribeanCampaign.moneyNeeds[0].id , ammount:5.3 , paymentId:'testpaymentId'}) ;
    ...
    // --------------- create connections ----------------------
        ...
        await db.userConnections.create({
            connectedUserId:caribbeanseaturtleproject.id,
